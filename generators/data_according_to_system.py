...
def generate_das_qvs(
    ...
    if 'tables' in schema_data:
        for table_name, table_info in schema_data['tables'].items():
            ...
            table_lines.append("    Load")
            table_lines.append("        MAX([modified_date]) As [max_modified_date]")
            table_lines.append("    From")
            table_lines.append("        [$(val__qvd_target)] (qvd)")
            table_lines.append("    ;")
            table_lines.append("")
            table_lines.append("    Let val__incremental_value = Coalesce(Peek('max_modified_date', -1, 'max_incremental_value'), '$(val__incremental_value)');")
            ...
            table_lines.append("")
            table_lines.append("Else")
            table_lines.append("    Trace No new records found, skipping append...;")
            table_lines.append("")
            table_lines.append("End If")
            table_lines.append("")
